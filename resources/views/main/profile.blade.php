...
@section('content')
<div class="bg-gradient-to-b relative py-8 lg:py-32">
                    ...
                    <div class="w-full lg:w-1/2">
                        <div class="flex flex-col items-center h-full gap-x-6 text-center lg:flex-row lg:items-start lg:text-left">
                            <div class="flex-shrink-0">
                                <img src="{!! $user->_avatar() !!}" alt="{{ $user->username }}" class="h-14 w-14 rounded-full lg:h-16 lg:w-16">
                            </div>
                            <div class="flex flex-col h-full">
                                <div class="flex-1">
                                    <h1 class=" mt-1.5 flex items-center justify-center font-semibold dark:text-gray-100 sm:text-lg lg:mt-0 lg:justify-start lg:text-2xl">{{ $user->name }}
                                        @if ($user->role=="admin")
                                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="mt-0.5 ml-3 inline h-5 w-5 text-primary-600 dark:text-white">
                                    ...
                                    <div class="my-2 space-y-1 text-xs text-muted md:text-sm dark:text-gray-300">
                                        <div class="text-center lg:text-left">Joined {{ $user->created_at->format('d F, Y') }}</div>
                                        <div>{{ $user->title_profile ?? 'Member active'  }}</div>
                                    </div>
                                    <div class="relative">
                    ...
                    <div class="w-full lg:w-1/2">
                        <div class="flex justify-center md:justify-end gap-2 md:gap-4">
                            <div class="animate-fade-in-left-bounce bg-primary-500/20 dark:text-primary-400 text-primary-600 flex flex-col gap-y-4 text-center justify-center md:justify-between py-4 md:py-10 h-40 md:h-56 rounded-xl w-40">
                                ...
                                <div>
                                    <div class="font-bold text-xl sm:text-2xl">{{ $user->_count_license() }}</div>
                            ...
                            <div class="animate-fade-in-left-bounce-2 bg-emerald-500/20 dark:text-emerald-400 text-emerald-600 flex flex-col gap-y-4 text-center justify-center md:justify-between py-4 md:py-10 h-40 md:h-56 rounded-xl w-40">
                                ...
                                <div>
                                    <div class="font-bold text-xl sm:text-2xl">{{ $user->_count_wishlist() }}</div>
